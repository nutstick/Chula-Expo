...
qrcode.decode = function(src){
    ...
    if(arguments.length==0)
    {
        var canvas_qr;
        try {
            canvas_qr = document.querySelector('qr-scanner-lite').shadowRoot.getElementById('qr-canvas');
        } catch(err) {
            try {
                canvas_qr = document.querySelector('qr-scanner').shadowRoot.getElementById("qr-canvas");
            } catch(err) {
                qrcode.result = err;
